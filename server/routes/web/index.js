module.exports = app => {
    ...
    // 引用模型
    // 运用到相对路径比较麻烦，所以使用mongoose来引用
    // const Article = require('../../models/Article')
    const mongoose = require('mongoose')
    // 由于我们已经已经在mongooes上引用了所有模型，所以我们可以直接取
    // 有两个参数，第一个名称，第二个是Schema
    ...
    router.get('/news/init', async (req,res) => {
        ...
        // 表示取纯粹的json，js对象或数组，不带mongooes的对象
        // Mongoose will, by default, "minimize" schemas by removing empty objects.
        const cats = await Category.find().lean().where({
            parent: parent
        })
        const newsTitles = ["鲁班七号-狮舞东方定制非遗文化狮头", "王者人生×王者荣耀 | 特权强势升级，来做王者峡谷最靓的仔！", "稷下星之队集结 快手官号回馈超值福利", "姐姐镜剪影曝光，曜往事大揭秘！", "新版本爆料⑥ | 美梦将成真~永久皮肤、回城特效免费得！", "心愿单升级完毕 距离心愿达成只差最后0.01公分", "6月29日全服不停机修复公告", "【已开服】6月26日正式服“稷下星之队”版本更新公告", "【正式服】“稷下星之队”版本异常问题说明", "【已开服】关于6月27日iOS区服开服延迟公告说明", "稷下精彩活动明日到来 任你徜徉王者峡谷", "稷下星之队集结 新版本超值福利回馈", "恭喜eStarPro捧起银龙杯 赛末冲刺惊喜礼不断", "王者大陆的端午宝藏活动公告", "峡谷庆端午 惊喜礼不断", "【6月15日 再战西安 · 2019年KPL春季赛总决赛重启公告】", "王者荣耀世界冠军杯荣耀来袭，KPL赛区选拔赛谁能突围而出？", "【关于2019年KPL春季赛总决赛门票退换及异地用户现场观赛补贴公告】", "虞姬世冠专属皮肤即将上线，世冠总奖池最高可达3200万", "世冠鱼乐战报第1期-KPL战队巡礼"]
        ...
        const cats = await Category.aggregate([
            ...
            // 定义我们只要几个
            {
                ...
                $addFields: {
                    // 就写newsList，修改字段
                    newsList: {
                        // 类似于数组的slice,第一个参数，要查的字段，第二个要查多少个
                        $slice: ['$newsList',5]
                    }
                }
            }
